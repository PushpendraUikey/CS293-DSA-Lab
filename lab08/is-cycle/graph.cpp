...
// Clever management of graph nodes!
...
void Graph::addNode(int v) {
    if (nodes.find(v) == nodes.end()) {
        nodes[v] = new GraphNode(v);    // new keyword dynamic allocate the memory hence returns pointer to it.
    }
}
...
void Graph::addEdge(int s, int t) {
    ...
    if(std::find(nodes[s]->adj.begin(), nodes[s]->adj.end(), t) != nodes[s]->adj.end())
        return; /// undirected graph if node is already have edge then we return
