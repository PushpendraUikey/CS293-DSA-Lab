...
bool tester::test_shortest_routes(void) {
    ...
    std::ifstream f_in("testcases/shortest_routes.txt");    // ifstream is the input file stream class from <fstream>
        // f_in is the instance of that class
    std::string line;
    std::string temp;
    ...
    // omitting the types <string, int> will result in the compilation error because compiler on
    // itself can't infer the types, someone has to explicitly tell them.
    ...
    // Using decltype(auto) is useful when you want the variable's type to exactly match the 
    // function's return type, without any modifications 
    // that might occur with plain auto (which can sometimes strip references)
    ...
    while (std::getline(q_in, line)) {
        ...
        std::istringstream iss(line);   // istringstream tokenize the line
//         >> Operator:
// This operator reads data from the stream until it hits whitespace or a delimiter, and stores it into
        while (iss >> temp) {
