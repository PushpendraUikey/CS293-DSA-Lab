...
std::vector<std::pair<int, std::string>> generateTestcaseStack(int num_operations) {
    std::vector<std::pair<int, std::string>> testcase;
    std::random_device rd;
    //Mersenne Twister random number generator (std::mt19937) with rd() as the seed, std::mt19937 is a standard, 
    //high-quality pseudorandom number generator commonly used for generating random numbers in C++.
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int> dis(0, 4);//a uniform integer distribution dis in the range [0, 4]
    std::uniform_int_distribution<int> dis2(-num_operations, num_operations);
    int stack_size = 0;
    for (int i = 0; i < num_operations; i++) {
        int operation = dis(gen);
