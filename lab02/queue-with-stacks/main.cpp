    ...
    for (std::pair<int, std::string> call : testcase) {
        if (call.second == "PUSH") {
            ...
            try {
                ...
                if (result != expected_value) {                                           /////// change
            ...
            duration += std::chrono::duration_cast<std::chrono::nanoseconds>(
                    stop - start).count();
            if (result != stl_stack.empty()) {                                           /////// change
...
bool checkTestcaseQueue(std::vector<std::pair<int, std::string>> testcase, 
    ...
    for (std::pair<int, std::string> call : testcase) 
    {
        if (call.second == "ENQUEUE") {
            ...
            duration += std::chrono::duration_cast<std::chrono::nanoseconds>(
                    stop - start).count();
            if (result != stl_queue.empty()) {                                           /////// change
            ...
            duration += std::chrono::duration_cast<std::chrono::nanoseconds>(
                    stop - start).count();
            if (result != stl_queue.size()) {                                           /////// change
...
std::vector<std::pair<int, std::string>> generateTestcaseStack(int num_operations) {
    std::vector<std::pair<int, std::string>> testcase;
    std::random_device rd;// provides a source of non-deterministic random numbers(from hardware, if available)
    std::mt19937 gen(rd());//mt19937 is a fast pseudo random number generator
    std::uniform_int_distribution<int> dis(0, 4); // a uniform number generator between 0 and 4 (inclusive)
    std::uniform_int_distribution<int> dis2(-num_operations, num_operations);//uniform number generator for operations
    int stack_size = 0;
    for (int i = 0; i < num_operations; i++) {
        int operation = dis(gen);
        switch(operation) {
