...
//ofstream &f: A reference to an ofstream object that is used to 
//write to a file. The tree data will be stored in this file.
...
void TREE::traverse(int typeOfTraversal){
    if(root==NULL)return;
    if(typeOfTraversal==0){ // 0 is for inorder traversal
        TreeNode* curr = root;  // temporary storage since making function call after changing root value
        ...
        root = curr;
    }else if(typeOfTraversal==1){ // 1 for post order traversal
        TreeNode * curr = root;
...
public:
    bool empty(){ //returns if the stack is empty
        // return (head!=NULL); // This was previous code, but was still working correct for given testcases?
        return (head == NULL) // modified
    }
    ...
    T top(){//returns the value of the most recently added element of the stack
        if(head==NULL) throw "Top called on empty stack";
        return head->val;
    }
};

// wrapper method to store the file.
...
void TREE::load(int flag, string filename, unordered_map<int, TreeNode*> &key_to_pointer) {
    ifstream f{filename};   // open paranthesis also works both calls the same constructor under the hood
    // But curly braces introduced in C++11 and they do list-initialization.
...
void TREE::printTREE(const string& prefix, bool isLeft)
{
    if( root != nullptr )
    {
        ...
        root = curr;        // this step is backtrack, back to the original state after call
    }
}
...
void TREE::getTREE(const string& prefix,  bool isLeft)
{
    if( root != nullptr )
    {
        ...
        // prefix itself is constant but the add operation here is making a temporary object as a result of
        // + operation (Allowed since we're not assigning anything to prefix)
        // In C++ temporaries can bind to const & !!!
