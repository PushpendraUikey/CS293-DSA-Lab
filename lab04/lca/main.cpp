...
int main(int argc, char **argv) {
  ...
  if( which == '1') {
    for(int j = 0; j < 10; j++ ) {
      ...
      TreeNode* x = nullptr;
        if(key_to_pointer.find(k1) != key_to_pointer.end()){
          ...
          // .at(key) in an unordered_map, it returns a reference to the actual value stored in the map not a copy
          // it is good practice as it allows to not insert the default value to the map!
        }
      TreeNode* y = nullptr;
    ...
    while(true){
      cout<<">>>";
      getline(cin,s);
      split(s,' '); // parse command line argument into tokens 
